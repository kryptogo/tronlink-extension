...
class Account {
  ...
  async update(
    ...
    const {
      data: { data: smartTokens },
    } = await axios
      .get(`${API_URL}/api/wallet/trc20_info`, {
    ...
    try {
      ...
      const filteredTokens = (account.assetV2 || []).filter(
        ({ value }) => value >= 0
      );
      for (const { key, value } of filteredTokens) {
        ...
        const filter = basicTokenPriceList.length
          ? basicTokenPriceList.filter(
              ({ first_token_id }) => first_token_id === key
            )
          : [];
        const trc20Filter = smartTokenPriceList.length
          ? smartTokenPriceList.filter(
              ({ fTokenAddr, sTokenAddr }) =>
                key === fTokenAddr && sTokenAddr === this.trxAddress
            )
          : [];
      ...
      for (const {
        ...
        const filter = smartTokenPriceList.filter(
          ({ fTokenAddr, sTokenAddr }) =>
            fTokenAddr === tokenAddress && sTokenAddr === this.trxAddress
        );
        const price = filter.length
          ? new BigNumber(filter[0].price).shiftedBy(-decimals).toString()
          : 0;
  ...
  async addSmartToken({ address, name, decimals, symbol }) {
    logger.info(
      `Adding TRC20 token '${address}' ${name} (${symbol}) to account '${this.address}'`
    );
  ...
  async sendBasicToken(recipient, amount, token) {
    ...
    try {
      ...
      const { transaction } = await NodeService.sunWeb.sidechain.trx.sendToken(
        recipient,
        amount,
        token,
        { privateKey: this.privateKey }
