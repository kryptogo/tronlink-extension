...
let getTokensMap = async function() {
  let { data } = await axios.get(
    `https://apilist.tronscan.org/api/token?showAll=1&limit=4000`
  );
...
export const app = {
  ...
  bindDuplexRequests() {
    ...
    this.duplex.on('setLanguage', (language) =>
      this.store.dispatch(setLanguage(language))
    );
    ...
    this.duplex.on('setSelectedBankRecordId', (id) =>
      this.store.dispatch(setSelectedBankRecordId(id))
    );

    this.duplex.on('changeDealCurrencyPage', (status) =>
      this.store.dispatch(changeDealCurrencyPage(status))
    );

    this.duplex.on('setAirdropInfo', (airdropInfo) =>
      this.store.dispatch(setAirdropInfo(airdropInfo))
    );

    this.duplex.on('setDappList', (dappList) =>
      this.store.dispatch(setDappList(dappList))
    );

    this.duplex.on('setAuthorizeDapps', (authorizeDapps) =>
      this.store.dispatch(setAuthorizeDapps(authorizeDapps))
    );

    this.duplex.on('setLedgerImportAddress', (address) =>
      this.store.dispatch(setLedgerImportAddress(address))
    );
